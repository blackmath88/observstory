...
## ADR-027: Observstory adds declared collaboration state, narrowly (issue #7)
**Decision.** The boundary is *repo observability + declared collaboration state*: sessions, gates, commitments and decisions confirmed by people and compared with repository evidence. Observstory doesn't become a project-operating surface, task manager, meeting recorder or calendar.
**Evidence.** Discovery Q1 (freeze or gate compared with activity: score 80, uncovered by any tool surveyed) and Q2 (commitments evaporate: 60). Trackers already solve PR ↔ task sync (Linear), and AI note-takers already solve drafting.
**Rejected.** Option 3 (an operating surface): it duplicates trackers and calendars and adds upkeep.

## ADR-028: Declared state lives in the repository as `.observstory/coordination.json`
**Decision.** Confirmed declarations are committed to the observed repository. The Action reads the file next to its checkout. It's optional: no file means no rail.
**Why.** The Action stays stateless. Declarations become versioned and reviewable in PRs, there's no second source of truth or hidden database, and agents can read them from the checkout.
**Cost.** Visible to everyone who can read the repository, which matters for public repos (K12). Confirming requires a commit.

## ADR-029: Extraction proposes; only a named human confirmation declares
**Decision.** The extractor writes `status: proposed`, with the source line. `observstory checkin confirm --by NAME` promotes items to `declared`, recording `confirmed_by` and `confirmed_at`. Reconciliation, the rail and the map ignore proposed items. The default extractor is deterministic, using explicit markers. An LLM extractor may be added behind the same interface, and it still only proposes. There's no recording or transcription in v1.
**Evidence.** Vendor-reported action-item hallucination above 35%; recording-consent law (D2, D3).
